...
public class ClassNumberOfMethods implements ClassExpectation {
    ...
    public void match(CKClassResult clazz, SheriffReport report) {
        if(!operator.compare(clazz.getNumberOfMethods())) {
            String message = String.format("class %s has a total of %d methods",
                    clazz.getClassName(), clazz.getNumberOfMethods());
